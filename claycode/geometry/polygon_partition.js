import { area, pickPointOnPerimeter, segmentPolygonIntersections, 
         isPointInPolygon, circularity} from "./geometry.js"
...
export function partitionPolygon(polygon, amounts) {
    ...
    while(amounts.length > 0) {
        ...
        if (amounts.length == 0) {
            ...
            if (res == null) { throw "Unable to split the polygon!" }
            const [pa, pb] = res
            partition.push(pa)
            to_split = pb
...
export function splitPolygonInSimplestPartition(polygon, partition_target_area_perc) {
    ...
    for(let i=0; i < MAX_TRIES; i++) {
        ...
        if (res == null) { continue } // bad cut

        let [pa, pb] = res
        for (let r = 0; r < 2; r ++ ) { // repeat twice to check also opposite combination
            const error_area = Math.abs(area(pa) - target_area_a) / Math.max(target_area_a, 1-target_area_a)
...
export function cutPolygon(polygon, cut_va_idx, va, cut_vb_idx, vb) {
    ...
    if (!isPointInPolygon(polygon, va.lerp(vb, 0.5))) {
